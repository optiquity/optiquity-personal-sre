...
> — a status command that never fetches cannot report a sync failure: it said *clean* while two
> days behind. Watch the **act** (a marker a successful sync rewrites), not the **state**.
