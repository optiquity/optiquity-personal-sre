...
- **A subagent's result is a report, not a side effect.** Use subagents to *learn* and
  *decide*; use the main operator to *act*. The same instinct is worth applying to unattended
  automation: a scheduled job that only *reports* is safe to run without a human, while one that
  acts needs a session — that's the **notify-only** posture in
